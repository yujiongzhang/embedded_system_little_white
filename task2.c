...
void pack_data(Message_Pressure* _msg){
    _msg->head = 0xa5;
    _msg->sum = _msg->head + _msg->pressure;
//    printf("sum = %d", _msg->sum); 
}

void process_input_data(Message_Pressure* _msg, char* _str){
    char str[100];
    strcpy(str, _str);

    //将字符串转换为 float
    _msg->pressure = atof(str); 
    pack_data(_msg);
}

void* thread_function_2() {

    char* file_path = "/home/zyj/embedded_system_little_white/data/pressure_data.csv";
    ...
    server_addr.sin_port = htons(55551);
    // 将 IP 地址从文本转换为二进制形式
    ...
    Message_Pressure m_message_body;
